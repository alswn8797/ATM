...
public class SavingAccount extends Account {
    ...
    //Constructor overwriting
    SavingAccount(double amount, String _openDate){
        //set account name
        accountName = "Saving Account";
        //set interest and dailyLimit
    ...
    //Constructor overwriting
    SavingAccount(double amount, Date _openDate){
        //set account name
        accountName = "Saving Account";
        //set interest and dailyLimit
        setInterest(interestRate);
    ...
    public String withdrawal(double amount, Date _issueDate) {
        String str="";
        //check requested amount is possible to withdraw
        try{
            ...
            if(checkWithdrawalAmount(amount)){
                ...
                //issueDate's date, month and year is same as current date,month and year
                if(issueDate.getDate()==new Date().getDate() && issueDate.getMonth()==new Date().getMonth() && issueDate.getYear()==new Date().getYear()){
                    usedDailyLimit += amount;
                }
    ...
    public void calcInterest() {
        ...
        //interest Rate is per annual. devide interest Rate by 365 for daily interest Rate
        float tmpInterest = getInterest()/365;
        
        //New date as now
        Date currentDate = new Date();
        //Declaring variation and set value as currendate's milliseconds
        long currentDateMs = currentDate.getTime();
        
        //loop until length of trasantions
        for (int i=0; i<(t.size()); i++) {
            //get issuedate of ith transaction in milliseconds
            long issueDateMs = t.get(i).issueDate.getTime();
            //get different from current time in milliseconds to issue date in milliseconds
            long diff = currentDateMs-issueDateMs;
            
            //diff is in milliseconds
            //divide 1000 to convert to second
            //devide 60 to convert to minute
            //devide 60 to convert to hours
            //devide 24 to convert to days
            //11days 20hours 23mins ... we only need 11days. I did ceiling
            double days = Math.ceil(diff/1000/60/60/24);
            
            //set ith of transaction's interestAmount
            t.get(i).interestAmount = t.get(i).amount * (days * tmpInterest);
        }
    }
}
